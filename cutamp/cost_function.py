...
class CostFunction:
    ...
    def kinematic_costs(self, rollout: Rollout) -> Union[dict, None]:
        ee_position = rollout["ee_position"]          # [B, K, 3]
        ee_quaternion = rollout["ee_quaternion"]      # [B, K, 4]
        desired_T = rollout["world_from_ee_desired"]  # [B, K, 4, 4]

        B, K = ee_position.shape[:2]

        desired_pose = Pose.from_matrix(desired_T.reshape(B * K, 4, 4))
        desired_position = desired_pose.position.reshape(B, K, 3)
        desired_quaternion = desired_pose.quaternion.reshape(B, K, 4)

        pos_errs = torch.sum((ee_position - desired_position).square(), dim=-1)

        ee_quaternion = torch.nn.functional.normalize(ee_quaternion, dim=-1, eps=1e-8)
        desired_quaternion = torch.nn.functional.normalize(desired_quaternion, dim=-1, eps=1e-8)

        dot = torch.sum(ee_quaternion * desired_quaternion, dim=-1)
        rot_errs = 1.0 - dot.square()
    ...
    def motion_costs(self, rollout: Rollout) -> Union[dict, None]:
        ...
        B, H = robot_spheres.shape[:2]
        with torch.profiler.record_function("coll::self_collision"):
            self.self_collision_cost_fn.setup_batch_tensors(
                batch_size=B,
                horizon=H,
            )
